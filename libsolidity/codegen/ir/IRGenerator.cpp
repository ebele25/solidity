...
string IRGenerator::generateGetter(VariableDeclaration const& _varDecl)
{
	...
	return m_context.functionCollector().createFunction(functionName, [&]() {
		...
		if (!paramTypes.empty())
			solAssert(location.second == 0, "Zero offset expected if there are parameters.");
		...
		{
			MappingType const* mappingType = dynamic_cast<MappingType const*>(returnType);
			ArrayType const* arrayType = dynamic_cast<ArrayType const*>(returnType);
			...
			parameters += keys;
			code += Whiskers(R"(
				slot := <indexAccess>(slot<?+keys>, <keys></+keys>)
			)")
			(
				"indexAccess",
				mappingType ?
			...
			returnType = mappingType ? mappingType->valueType() : arrayType->baseType();
		}
